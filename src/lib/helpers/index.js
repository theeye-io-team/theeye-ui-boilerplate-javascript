...
const Helpers = {
   ...
   * @param {String} alignment HORIZONTAL_SEGMENTS, VERTICAL_SEGMENTS
   */
  segmentsIntersectionRatio (bbox1, bbox2, alignment) {
    if (bbox1 === bbox2) { return true }

    let rectBbox1 = Helpers.rectifyBBox(JSON.parse(JSON.stringify(bbox1)))
    let rectBbox2 = Helpers.rectifyBBox(JSON.parse(JSON.stringify(bbox2)))
